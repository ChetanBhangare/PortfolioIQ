...
import {analyzePortfolio,analyzeRegimes,analyzeRisk,optimizePortfolio} from "@/lib/api";
...
const runAnalysis=useCallback(async()=>{setStatus("loading");setError(null);setAnalysis(null);try{const [performance,risk,optimization,regime]=await Promise.all([analyzePortfolio(portfolio),analyzeRisk(portfolio),optimizePortfolio(portfolio),analyzeRegimes(portfolio)]);setAnalysis({portfolio:performance,risk,optimization,regime});setStatus("success");return true}catch(value){setStatus("error");setError(value instanceof Error?value.message:"Unexpected analytics error.");return false}},[portfolio]);
